...
def load_dataset(train_dir, dev_dir, stemming, lower_case):
    X0 = loadDir(train_dir + '/ham/',stemming, lower_case)
    X1 = loadDir(train_dir + '/spam/',stemming, lower_case)
    X = X0 + X1
    Y = len(X0) * [1] + len(X1) * [0]
    Y = np.array(Y)

    X_test0 = loadDir(dev_dir + '/ham/',stemming, lower_case)
    X_test1 = loadDir(dev_dir + '/spam/',stemming, lower_case)
